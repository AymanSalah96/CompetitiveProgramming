...
Idea:
- My approach is to maximize the lower left and minimize the upper right corners on the two windows.
- If the new corners can make a valid rectangle then the answer will be these corners.
- Otherwise the answer is "No Overlap" (there is no common area).
*/
...
int main() {
	...
	for(int T = 1; T <= t; T++) {
		...
		if (left_x < right_x && lower_y < upper_y)
			cout << left_x << " " << lower_y << " " << right_x << " " << upper_y << endl;
		else
